...
from runtime_store.config_drift import find_config_drift
...
def test_find_config_drift_reports_missing_consumer():
    example = {
    ...
    items = find_config_drift(example, actual)
    assert len(items) == 1
    assert items[0].path == "flexible_consumers[id=swimspa]"
